...
function Facebookbot(configuration) {
        ...
        bot.send = function(message, cb) {
            ...
            request({
                ...
                function(err, res, body) {
                    ...
                    if (body.error) {
                        botkit.debug('API ERROR SOURCE', message)
    ...
    facebook_botkit.createWebhookEndpoints = function(webserver, bot, subpath, cb) {
        if (cb == 'undefined' && typeof subpath == 'function') {
          cb = subpath;
          subpath = 'receive';
        }
        ...
        webserver.post('/facebook/'+subpath, function(req, res) {
            res.send('ok');
            facebook_botkit.handleWebhookPayload(req, res, bot);
        });

        webserver.get('/facebook/'+subpath, function(req, res) {
    ...
    facebook_botkit.middleware.categorize.use(function handleOptIn(bot, message, next) {
        ...
        if (message.game_play) {
            message.type = 'facebook_game_play';
        }
